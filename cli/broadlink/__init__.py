...
def gendevice(devtype, host, mac):
  DEVICES = {
      0: sp1,  # SP1
      0x2711: sp2,  # SP2
      0x2719: sp2,  # Honeywell SP2
      0x271a: sp2,  # Honeywell SP2
      0x7919: sp2,  # Honeywell SP2
      0x791a: sp2,  # Honeywell SP2
      0x2720: sp2,  # SPMini
      0x753e: sp2,  # SP3
      0x2728: sp2,  # SPMini2
      0x2733: sp2,  # OEM branded SPMini
      0x273e: sp2,  # OEM branded SPMini
      ...
      0x272a: rm,  # RM2 Pro Plus
      0x2787: rm,  # RM2 Pro Plus2
      0x278b: rm,  # RM2 Pro Plus BL
      0x278f: rm,  # RM Mini Shate
      0x2714: a1,  # A1
      0x4EB5: mp1,  # MP1
  }
  if devtype in DEVICES:
    return DEVICES[devtype](host=host, mac=mac)
  if devtype >= 0x7530 and devtype <= 0x7918:  # OEM branded SPMini2
      ...
      while True:
        try:
          ...
          if (time.time() - starttime) > self.timeout:
            raise
    # print('[debug] encoded response: %s' % ''.join(format(x, '02x') for x in bytearray(response[0])))
...
class rm(device):
  ...
  def enter_frequency_scan_and_learn(self):
    packet = bytearray(16)
    packet[0] = 0x19
    self.send_packet(0x6a, packet)

  def check_frequency_scan_completed(self):
    packet = bytearray(16)
    packet[0] = 0x1a
    response = self.send_packet(0x6a, packet)
    err = response[0x22] | (response[0x23] << 8)
    if err == 0:
      ...
      # print('[debug] sweep stage one: %s' % ''.join(format(x, '02x') for x in data))
      return data[0] == 0x01
    else:
      print 'Error response'
    return False

  def enter_learning_after_frequency_scan(self):
    packet = bytearray(16)
    packet[0] = 0x1b
    response = self.send_packet(0x6a, packet)
    err = response[0x22] | (response[0x23] << 8)
    if err == 0:
      payload = self.decrypt(bytes(response[0x38:]))
      data = bytearray(payload[4:])
      # print('[debug] sweep stage two: %s' % ''.join(format(x, '02x') for x in data))
      return data[0] == 0x01
    else:
      print 'Error response'
    return False
